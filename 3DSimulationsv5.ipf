...
function Model3DPanel()
	...
	if(svar_exists(extralist))
		...
		variable /g startq = 0.04, endq = 0.4
	endif
	...
	//tab 3 - CyRSoXS
	TitleBox CyrsoxsNewVer,pos={130,100},size={276,16},disable=1,title="Analyze CyRSoXS HDFs (>= v1.0)"
	TitleBox CyrsoxsNewVer,labelBack=(56576,56576,56576),fSize=14,frame=0
	TitleBox CyrsoxsNewVer,fColor=(65280,0,0)
	Button Analyze_HDF_new,pos={140,135},size={75,35},fcolor=(255,165,0),help={"Load single simulation\rCompatible with output generated by CyRSoXS v1.0+\rGive path to simulation folder\r(folder containing CyRSoXS HDF output directory, morphology hdf5 & config file)\rFunction first prompts for the h5 scattering datafiles\rSecond prompt is for the morpohlogy hdf5 file to read dataset dimensions and physsize"},disable=1,proc=Analyze_HDF_but_new,title="\\Z11Load single"
	Button Analyze_HDF_batch_new,pos={245,135},fcolor=(152,251,152),size={75,35},help={"Batch load sims\rGive path to parent folder containing all simulation folders\rAlso executes 'Generate 1D profiles'"},disable=1,proc=Analyze_HDF_but_batch_new,title="\\Z11Load batch"
	Button generate1D,pos={140,200},size={75,35},fcolor=(250,128,114),help={"Generate Perpendicular (Perp1D), Parallel (Para1D), A(q) (Aniso1D) and A(energy) (IntAniso) waves\rPerp and Para with respect to incident X-ray polarization\rOnly use with 'Load Single'\rMust set data folder to root:packages:HDFdata:<simname>"},disable=1,proc=generateall1D_but,title="\\Z09Generate\r1D profiles"
	SetVariable startq,pos={50,250},size={170,25},help={"Enter the lower integration q limit for A(energy)"},disable=1,title="Start q [nm^(-1)]"
...
//function first prompts for the h5 scattering datafiles
//second prompt is for the morpohlogy hdf5 file to read dataset dimensions and physsize
	string pathbase
	...
	//store the path of the last simulation loaded
	string/G root:Packages:ScatterSim3D:lastsimload = foldersave
	svar lastsimload = root:Packages:ScatterSim3D:lastsimload
	lastsimload = getdatafolder(1)
...
Function Analyze_HDF_but_batch(ba) : ButtonControl
	STRUCT WMButtonAction &ba
...
Function Analyze_HDF_but_batch_new(ba) : ButtonControl
	STRUCT WMButtonAction &ba
...
Function generateall1D_but(ba) : ButtonControl
	STRUCT WMButtonAction &ba
	switch( ba.eventCode )
		case 2: // mouse up
			// click code here
...
function generate_all1d()
//generates Perpendicular (Perp1D), Parallel (Para1D), A(q) (Aniso1D) and A(energy) (IntAniso) waves 
//Perp and Para are with respect to incident X-ray polarization
	nvar startq = root:packages:ScatterSim3D:startq
	nvar endq = root:packages:ScatterSim3D:endq
	wave/Z enewave = Envalues
	variable ene, j
	wave/Z para, perp, anisoq, ratio3dtemp, perp3dvsenhdf, para3dvsenhdf
	make/O/N=(numpnts(enewave)) IntAniso
	j = 0
		wave/Z anisoen_x, anisoen_y
		string enes
		for (j = 0; j < numpnts(enewave); j+=1)
			enes = replacestring(".",num2str(enewave[j]),"_")
			string paraname = "Para1D"+enes
			string perpname = "Perp1D"+enes
			string anisoname = "Aniso1D_"+enes
			duplicate/o/RMD=[][j] para3dvsenhdf $paraname
			duplicate/o $paraname para
			duplicate/o/RMD=[][j] ratio3dtemp $anisoname
			duplicate/o $anisoname anisoq
			duplicate/o/RMD=[][j] perp3dvsenhdf $perpname
			duplicate/o $perpname perp
			IntAniso[j] = (area($paraname,startq,endq)-area($perpname,startq,endq))/(area($paraname,startq,endq)+area($perpname,startq,endq))
			//print enewave[j], IntAniso[j]
		endfor
	killwaves/Z para, perp, anisoq
end

function load_all_sim()
//Load all simulations in a particular folder. 
//Works for output generated by old CyRSoXS versions (pre v1.0)
//Each simulation results folder should contain the config file, morphology, and HDF5 output folder
	newpath /M="Path for simulation results"/O/Q/Z dirpathpath
	if(v_flag)
		print "bad path entered, stopping"
	endif
	pathinfo dirpathpath
	string pathtoload
	string list_of_dirs = IndexedDir(dirpathpath,-1,1)
	variable i, startq, endq
	for(i=0;i<itemsinlist(list_of_dirs);i++)					//Load each simulation in folder
		pathtoload = stringfromlist(i,list_of_dirs)
		Analyze_HDF5_dir(pathbase = pathtoload)
		generate_all1d()
		print pathtoload		
	endfor
	print "Loaded ", itemsinlist(list_of_dirs), " folders" 
end

function load_all_sim_new()
//Load all simulations in a particular folder. 
//Works for output generated by old CyRSoXS versions (from v1.0 onwards)
//Each simulation results folder should contain the config file, morphology, and HDF5 output folder
	newpath /M="Path for simulation results"/O/Q/Z dirpathpath
	if(v_flag)
		print "bad path entered, stopping"
	...
	for(i=0;i<itemsinlist(list_of_dirs);i++)					//Load each simulation in folder
		pathtoload = stringfromlist(i,list_of_dirs)
		Analyze_HDF5_dir_new(pathbase = pathtoload)
		setDataFolder lastsimload
		generate_all1d()
		print pathtoload		
	endfor
	print "Loaded ", itemsinlist(list_of_dirs), " folders" 
