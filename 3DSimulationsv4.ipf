...
//	wave /d/c np // 2D complex index of refraction projection (or slice) (along x axis) // depricated in this version, not useful
	...
	variable en // current energy = 1242 / wavelength =  ( 1242/(2*pi)) * wavevector
	variable wavelength // current wavelength = 1242 / energy = 2*pi / wavevector (in nanometers)
	variable k // current wavevector magnitude (2*pi / wavelength) = (2 * pi / 1242 ) * Energy (in inverse nanometers)
...
//	print "Save3D="+num2str(save3d)
	...
	addtologbook(s3d,"Command to run again: " + cmdstr)
...
//	endif 
	...
	for( j=0; j <dimsize(enwave,0) ; j+=1)
		...
		for(angle=0;angle<180;angle+=anglestep)
			offsetx = floor(enoise(dimsize(s3d.m1,1)/10))
			offsety = floor(enoise(dimsize(s3d.m1,2)/10))
			...
			if(result<0)
				...
				return -1
			elseif(result>0 && s3d.en == energymin)
		...
		//	addtologbook(s3d,"Angle="+num2str(angle))
			...
			if(angle/10 == round(angle/10) && angle + anglestep < 180)
				if(angle>0)
					...
					createsimlayout()
					TextBox/w=Simulation_Layout/C/N=text5/F=0/A=LB/X=33.80/Y=91.12 "\\F'Arial Black'\\Z24"+num2str(s3d.en)+"eV"
...
function Calculate3DScatter(s3d)
	...
	FFT /WINF=Hamming /pad={1*thicknum,1*num,1*num} /Dest=pxfft px
	FFT /WINF=Hamming /pad={1*thicknum,1*num,1*num} /Dest=pyfft py
	FFT /WINF=Hamming /pad={1*thicknum,1*num,1*num} /Dest=pzfft pz
...
function CalculatePolarizationWave(s3d)
	...
	for(i=0;i<matnum;i+=1)
		...
		if(ocsexist(material)==2)
			delpara = getdelta(material,1242/s3d.en,aligned=1)
			betapara = getbeta(material,1242/s3d.en,aligned=1)
			delperp = getdelta(material,1242/s3d.en,aligned=0)
			betaperp = getbeta(material,1242/s3d.en,aligned=0)
			result = 0
		elseif(ocsexist(material)==1)
			delpara = getdelta(material,1242/s3d.en)
			betapara = getbeta(material,1242/s3d.en)
			delperp = getdelta(material,1242/s3d.en)
			betaperp = getbeta(material,1242/s3d.en)
			result = i+1
		else
...
window Simulation_Layout() : Layout
	...
	Layout/P=Landscape/C=1/K=1/W=(100,50,1100,800) ScatteringIntensity(378,54,576,594)/O=1/F=0
	ModifyLayout mag=1
...
function ScatteringPatterndisp()
	...
	ModifyImage/w=ScatteringPattern  scatter3D ctab= {0.00001,30000,Terrain,0}
...
function BetaProjectiondisp()
	...
	ModifyImage/w=BetaProjection  pzimagproj ctab= {*,*,Grays256,0}
	...
	setaxis /w=BetaProjection left 0,*
	setaxis /w=BetaProjection top 0,*
	ColorScale/w=BetaProjection /C/N=text0/F=0/A=MC/X=44.20/Y=6.15 image=pzimagproj, nticks=2, lowTrip=1e-05
End
...
function DeltaProjectiondisp()
	...
	ModifyImage/w=deltaprojection pzrealproj ctab= {*,*,Grays256,0}
	...
	setaxis /w=deltaprojection left 0,*
	setaxis /w=deltaprojection top 0,*
	ColorScale/w=deltaprojection/C/N=text0/F=0/A=MC/X=41.90/Y=5.59 image=pzrealproj, nticks=2, minor=1
End
...
function /s variables_lamella()
	string variables
	variables = "Nucleation Probability [%],SetVariable,5e-4;"
	//variables = variables + "Xtal Thickness [px],SetVariable,5;" get from size parameter
	variables = variables + "Xtal thickness in lamella [%],SetVariable,.3;"
	variables = variables + "Xtal variability sigma [px],SetVariable,.3;"
	//variables = variables + "Amorphous thickness [px],SetVariable,3;"
	variables = variables + "Amorph Alignment [-1 1][L ||],SetVariable, 0"
	variables = variables + "Amorph Alignment width [σ],SetVariable,0"
	...
	variables = variables + "Intra-to-Inter lamella growth factor,SetVariable,20;"
	variables = variables + "Lamella region volume [%},SetVariable,90;"
	variables = variables + "Amorphous region thickness [%},SetVariable,90;"
	variables = variables + "Material 2 in amorphous region [%},SetVariable,50;"
	return variables
end
...
function model3D_lamella(s3d)
	...
	variable xtallampct = 		str2num(stringfromlist( 1 ,s3d.paramstring,","))/100 
	...
	variable lamthickness = s3d.size * xtallampct//str2num(stringfromlist( 1 ,s3d.paramstring,","))
	variable amorphthickness = 	s3d.size * (1-xtallampct)//str2num(stringfromlist( 3 ,s3d.paramstring,","))
	variable nxy = s3d.num
	
	// outputs are m and density
...
//	SetActiveSubwindow _endfloat_
	...
	// loop through edge points
		...
		if(percentdone-lastpercent > 1) //doupdate
			...
			if(s3d.movie)
				if(itteration==1)
					tempalignmap(alignx,aligny,alignz,alignmag,3,0)
					ModifyGraph rgb=(3,52428,1)
					//appendimage thickness
					//ModifyImage thickness plane=3
				else
					tempalignmap(alignx,aligny,alignz,alignmag,3,1)
				endif
				doupdate
	...
	duplicate /o nonxtal, tempdens 
	tempdens = 1-(1- regiondens/100)*(1-nonxtal) // now this is an overall density filter for the film (no change in crystalline regions, some percent change in amorphous regions)
	...
	make /n=(s3d.thickness,s3d.num,s3d.num,4) /o m1=mat,m2=0, m3=0
	m1[][][][3] *=(1-mat2inamorph)
	m2[][][][3] =mat2inamorph * mat[p][q][r][3]// assigning half of the unaligned portion to the second material (setting materail 2 to the same as m1 is default)
	make /n=(s3d.thickness,s3d.num,s3d.num) /o density1 = m1[p][q][r][0]^2 +m1[p][q][r][1]^2 +m1[p][q][r][2]^2 + m1[p][q][r][3] // can make different density for aligned vs unaligned
	make /n=(s3d.thickness,s3d.num,s3d.num) /o density2 = m2[p][q][r][3]
	m3[][][][3] = 1-density1[p][q][r]-density2[p][q][r] // the remaining space will fill with material 3 (usually this would be vacuum, so this is a film thickness mask)
	
	wave s3d.m1=m1, s3d.m2=m2, s3d.m3=m3
	wave s3d.density1 = density1
	wave s3d.density2 = density2
	
	//second material needs to be vacuum
	// this returns the material matrix
end
...
function scatterimage(folder,en,[addtolayout])
	...
	SetAxis /w=$sgname left 0.00001,50
	...
	ModifyGraph /w=$sgname grid=1,tick=2,gfSize=20,axThick=2,standoff=0,gridStyle=3,gridRGB=(43690,43690,43690),useNegRGB($para1Dname)=1,usePlusRGB($para1Dname)=1,plusRGB=(65535,0,0),negRGB($para1Dname)=(3,52428,1)
...
//	duplicate /o M_InterpolatedImage, w0
	...
	imagerotate /o/E=(nan)/A=(angle) wtot
	imagerotate /o/E=(nan)/A=(angle) w0
...
//	imagefilter /n=5/o hybridmedian w3
	...
	multithread w0 /= sqrt(wtotnew[p][q][r])
	multithread w1 /= sqrt(wtotnew[p][q][r])
	multithread w2 /= sqrt(wtotnew[p][q][r])
	multithread w3 /= wtotnew[p][q][r]
...
//			nvar useprecalcalignment
			...
			model3D(morphology,voxelsize,sizescale,resolution,thickness,paramstring,materialstring,alignmentstring,ycomp+","+zcomp,startxrayenergy,endxrayenergy,numensteps,movie = movie, save3d = savescattering2D, rotsys = rotatesys)
			setdatafolder foldersave
			break
		case -1: // control being killed
			break
